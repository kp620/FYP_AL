...
# Load IDS IID Multiclass dataset
def load_data():
    print("Loading data...")
...
def self_PCA(full_dataset, d = 3):
    """
    Input: full dataset, d dimension
    Output: PCA dataset, eigenvectors
    """
...
# Kmeans++ algorithm
def Kmeansplusplus(full_dataset_pca, K=15):
    """
    Input: PCA dataset, K clusters
    Output: K centroids
    """
...
def mini_K(centroids, full_dataset_pca, max_iters= 10, rs_rate=0.05, K=15):
    """
    Input: centroids, PCA dataset, max_iters, random sampling rate, K clusters
    ...
    for t in range(max_iters):
        ...
        for i, x in enumerate(M):
            c_index = d[i]
            v[c_index] += 1
            # Update learning rate for this center
            n = 1 / v[c_index]  
            # Take gradient step
...
def cluster_sample(full_dataset, full_dataset_pca, C, rs_rate = 0.05):
    """
    Input: full dataset, PCA dataset, centroids, random sampling rate
    Output: selected indices & not selected indices
    """
    x_data = full_dataset_pca.tensors[0].numpy()
...
def main(rs_rate = 0.001, d = 10, K = 15):
    """
    Input: random sampling rate, PCA dimension, number of clusters
    """
    ...
    print('Mini-batch Kmeans finished!')
