...
def explain_model(class_type):
    model = ResNet18_1D(num_classes=3)
    # print(model)
...
def train(model, train_loader, device, dtype, criterion, learning_rate, classifer, optimizer):
    model.train()
...
def gradient_train(model, unlabel_loader, pseudo_labels, device, dtype, batch_size, criterion, classifer):
    gradients = []
    model = model.to(device=device)
